...
entries$DTSTART = as_datetime(entries$DTSTART)
entries$DTEND = as_datetime(entries$DTEND)
entries$DTSTAMP = as_datetime(entries$DTSTAMP)
entries$CREATED = as_datetime(entries$CREATED)
entries$`LAST-MODIFIED` = as_datetime(entries$`LAST-MODIFIED`)

#### Save CSV ####
write.csv(entries, .out_dir, row.names = FALSE)
